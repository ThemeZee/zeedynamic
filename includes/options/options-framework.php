...
function themezee_options_add_theme_page() {
	add_theme_page(__('Theme Options', 'zeeDynamic_language'), __('Theme Options', 'zeeDynamic_language'), 'edit_theme_options', 'themezee', 'themezee_options_page');
}
...
function themezee_options_scripts() { 
	...
	if ( isset($_GET['page']) and $_GET['page'] == 'themezee' ) :
		...
		wp_localize_script('themezee_options_js', 'zee_localizing_upload_js', array(
			'use_this_image' => __('Use this Image', 'zeeDynamic_language')
		));
...
?>
	...
	<div class="wrap themezee-admin-wrapper">  			
		...
		<div id="themezee-admin-header" class="themezee-admin-clearfix">
			...
			<h2 class="themezee-admin-header-title"><?php echo $theme_data->Name; ?> <?php _e('Theme Options', 'zeeDynamic_language'); ?></h2>
		...
		<div id="themezee-admin-wrap" class="themezee-admin-clearfix">
			...
			<?php if ( isset( $_GET['settings-updated'] ) ) : ?>
				<div class="themezee-updated"><?php _e('Theme Options successfully updated.', 'zeeDynamic_language'); ?></div>
			<?php endif; ?>
		...
		<?php 
			...
			?>
				...
				<form class="themezee-admin-form" action="options.php" method="post">
					...
					<p><input name="Submit" class="button-primary" type="submit" value="<?php esc_attr_e('Save Changes', 'zeeDynamic_language'); ?>" /></p>
				</form>
...
function themezee_options_tabs_menu() {
	...
	$links = array();
	if ( $current == 'welcome' ) :
		$links[] = '<a class="nav-tab nav-tab-active" href="?page=themezee&tab=welcome">'. __('Welcome', 'zeeDynamic_language') .'</a>';
	else :
		$links[] = '<a class="nav-tab" href="?page=themezee&tab=welcome">'. __('Welcome', 'zeeDynamic_language') .'</a>';
	endif;
...
function themezee_options_display_setting( $setting = array() ) {
	...
	switch ( $setting['type'] ) {
		...
		case 'image':
			...
			echo '<input class="zee-upload-image-button button-secondary" type="button" value="'. __('Upload Image', 'zeeDynamic_language') .'" />';
			echo '	<label>'.$setting['desc'].'</label>';
		break;
