...
if ( ! function_exists( 'themezee_display_postmeta' ) ) :
	...
	function themezee_display_postmeta() { ?>
		...
		<?php printf(__('Posted on <a href="%1$s" title="%2$s" rel="bookmark"><time datetime="%3$s">%4$s</time></a>', 'zeeDynamic_language'), 
		...
		<?php printf(__('by <a href="%1$s" title="%2$s" rel="author">%3$s</a>', 'zeeDynamic_language'), 
				esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ),
				esc_attr( sprintf( __( 'View all posts by %s', 'zeeDynamic_language' ), get_the_author() ) ),
				get_the_author()
			);
		?>
		</span>
	...
	<?php if ( comments_open() ) : ?>
		<span class="meta-comments sep">
			<?php comments_popup_link( __('Leave a comment', 'zeeDynamic_language'),__('One comment','zeeDynamic_language'),__('% comments','zeeDynamic_language') ); ?>
		</span>
	<?php endif; ?>
	<?php
		edit_post_link(__( 'Edit Post', 'zeeDynamic_language' ));
	}
	...
	function themezee_display_postinfo_single() {
		...
		if ( $tag_list ) : ?>
			<span class="meta-tags">
				<?php printf(__('tagged with %1$s', 'zeeDynamic_language'), $tag_list); ?>
			</span>
	<?php 
		endif; 
	?>
		...
		if ( $wp_query->max_num_pages > 1 ) :
			...
			if(function_exists('wp_pagenavi')) : // if PageNavi is activated
				...
				<div class="post-pagination clearfix">
					<span class="post-pagination-alignleft"><?php next_posts_link(__('&laquo; Older Entries', 'zeeDynamic_language')) ?></span>
					<span class="post-pagination-alignright"><?php previous_posts_link (__('Recent Entries &raquo;', 'zeeDynamic_language')) ?></span>
				</div>
	...
	?>
		...
		<div class="frontpage-category-wrapper">
			...
			<h2 class="frontpage-category-title">
				<?php printf( __( '<span>Latest in</span> %s', 'zeeDynamic_language' ), get_category_by_slug(esc_attr($category))->cat_name); ?>
			<h2>
...
if ( ! function_exists( 'themezee_display_category_posts_columns' ) ):
	...
	?>
		...
		<div class="frontpage-category-wrapper clearfix">
			...
			<div class="frontpage-category-left frontpage-category-columns">
				...
				<h2 class="frontpage-category-title">
					<?php printf( __( '<span>Latest in</span> %s', 'zeeDynamic_language' ), get_category_by_slug(esc_attr($options['themeZee_frontpage_category_three']))->cat_name); ?>
				<h2>
	...
	?>
			<div class="frontpage-category-right frontpage-category-columns">
				...
				<h2 class="frontpage-category-title">
					<?php printf( __( '<span>Latest in</span> %s', 'zeeDynamic_language' ), get_category_by_slug(esc_attr($options['themeZee_frontpage_category_four']))->cat_name); ?>
				<h2>
				...
				<?php while ($zee_slider_query->have_posts()) : $zee_slider_query->the_post(); ?>
					...
					<li id="slide-<?php the_ID(); ?>" class="zeeslide">
						...
						<div class="slide-entry clearfix">
							...
							<?php the_excerpt(); ?>
							<a href="<?php esc_url(the_permalink()) ?>" class="slide-more-link"><?php _e('Read more &raquo;', 'zeeDynamic_language'); ?></a>
						</div>
